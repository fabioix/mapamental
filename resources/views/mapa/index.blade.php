@extends('layouts.app')
@section('title', 'Mapa Mental')

@section('content')
    <div class="sair">
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" type="submit">Sair</button>
        </form>
    </div>

    <div class="grid place-content-center h-auto">
        <div class="py-4">
            <form action="{{ route('mapa.novo') }}" method="POST">
                @csrf
                <button class="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded"
                    style="margin-left:82px" type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                        fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                        <path
                            d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z" />
                        <path fill-rule="evenodd"
                            d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z" />
                    </svg></button>
            </form>
        </div>
        <?php
        $linha = 1;
        ?>
        
        <div id="corp">
            <div class="flex">
                @csrf
                @foreach ($mapa as $mapas)
                    @if ($width = $mapas->child * 200 == 0)
                        <?php $width = 200; ?>
                    @else
                        <?php $width = $mapas->child * 200; ?>
                    @endif

                    @include('mapa.constructor.cards')
                    <?php
                    $linha = $mapas->lin;
                    ?>
                @endforeach
            </div>
        </div>
    </div>
@endsection

<script>
    function save(id) {
        var name = 'formsave' + id;
        document.getElementById(name).submit();
    }
    function margin(){
        var margin = -(document.getElementById("texto1").offsetWidth + 20)/2 + 100; 
        document.getElementById("corp").style = 'margin-left:' + margin;
    }
</script>
